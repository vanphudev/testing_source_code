...
When("Tôi gửi yêu cầu Http PUT đến Endpoint {string} với dữ liệu người dùng đọc từ Excel", async function (endpoint) {
   ...
   try {
      ...
      if (this.authorization === null) {
         res = await chai.request(server).put(endpoint).set("client_id", this.client_id).send(this.inputData);
      }
      if (this.client_id === null) {
         res = await chai.request(server).put(endpoint).set("authorization", this.authorization).send(this.inputData);
      }
      if (this.client_id === null && this.authorization === null) {
         res = await chai.request(server).put(endpoint).send(this.inputData);
      }
      if (this.client_id !== null && this.authorization !== null) {
         res = await chai
            .request(server)
