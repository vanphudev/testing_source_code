...
Then("Tôi mong muốn dữ liệu trả về từ API endpoint phải đúng với định dạng JSON", function () {
   expect(this.response).to.be.json;
   const responseData = this.response.body;
   this.attach("Dữ liệu trả về: " + JSON.stringify(responseData), "application/json");
});

Then(
   "Kiểm tra các trường thông tin trả về từ API endpoint phải có thông tin người dùng đã đăng nhập thành công với Token và Refresh Token",
   function () {
      const responseBody = this.response.body;
      expect(responseBody).to.have.property("status", 200);
