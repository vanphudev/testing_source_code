Feature: Remove Cart Item
   ...
   Scenario Outline: Xóa sản phẩm ra khỏi giỏ hàng của người dùng
      ...
      And Tôi mong muốn dữ liệu trả về từ API endpoint phải đúng với định dạng JSON đồng thời kiểm tra các trường thông tin trả về từ API endpoint phải có thông tin người dùng đã xóa sản phẩm khỏi giỏ hàng thành công.
